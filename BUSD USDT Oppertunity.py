...
def BUYBUSD():
    order = client.order_market_buy(symbol=BUSDUAH, quantity=key.size)
    log.write(str(order) + "\n")
    order = client.order_market_sell(symbol=BUSDUSDT, quantity=key.size)
    log.write(str(order) + "\n")
    order = client.order_market_sell(symbol=USDTUAH, quantity=key.size)
    log.write(str(order) + "\n")

def BUYUSDT():
    order = client.order_market_buy(symbol=USDTUAH, quantity=key.size)
    log.write(str(order) + "\n")
    order = client.order_market_buy(symbol=BUSDUSDT, quantity=key.size)
    log.write(str(order) + "\n")
    order = client.order_market_sell(symbol=BUSDUAH, quantity=key.size)
    log.write(str(order) + "\n")
